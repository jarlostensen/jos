...
typedef struct page_table_flags_struct page_table_flags_t;
...
uint32_t k_virt_to_phys(pd_handle_t pd_, uint32_t virt)
{
    ...
    printf("\tpd = 0x%x, pt starts at 0x%x ", pd, pd->_phys_address);
    if( pd->_present )
    {
        size_t pt_offset = (virt - (virt/kPageDirEntryRange) * kPageDirEntryRange)/kFrameSize;
        page_table_t* pt = (page_table_t*)(pd->_phys_address + pt_offset);
        printf("offset = 0x%x, pt = 0x%x, present = %d ", pt_offset, pt, pt->_present);
        if(pt->_present)
        {
            return pt->_phys_address;
        }
        else
        {
...
void k_paging_init()
{
    ...
    printf("\nallocated pd @ 0x%x, first pt @ 0x%x\n", _k_page_dir,_k_page_dir->_phys_address);    
    ...
    // make it so
    printf("k_paging_init enabling paging, page dir @ 0x%x, 0x%x...", _k_page_dir, _k_page_dir->_phys_address);
    //_k_load_page_directory((uint32_t)_k_page_dir);
    //_k_enable_paging();  
    printf("ok\n");
    uint32_t map = k_virt_to_phys(_k_page_dir,0x10021c);
